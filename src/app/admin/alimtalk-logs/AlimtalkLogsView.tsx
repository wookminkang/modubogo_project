...
/**
 * 알림톡 발송 이력 본문 (클라이언트).
 * 전체 로그는 useSuspenseQuery 로 받고, 성공/실패 탭은 서버에서 searchParams 로
 * 해석한 activeTab 으로 필터링한다.
 */
export default function AlimtalkLogsView({
  activeTab,
}: {
  activeTab: "success" | "failed";
}) {
  const { data: allLogs } = useSuspenseQuery(alimtalkLogsQuery());
