...
/**
 * 대시보드 본문 (클라이언트).
 * 원자료는 useSuspenseQuery 로 받고, 시간 의존 계산은 서버가 고정해 넘긴 nowMs 를 사용해
 * 하이드레이션 불일치를 피한다. currentMonth 는 서버에서 searchParams 로 이미 해석된 값.
 */
    ...
    .flatMap((r) =>
      (r.validity_items as { subject: string; expiry_date: string }[]).map(
        (v) => ({
          ...
          daysLeft: Math.ceil(
            (new Date(v.expiry_date).getTime() - nowMs) / 86400000,
          ),
        }),
      ),
    )
