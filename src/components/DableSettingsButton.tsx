...
export default function DableSettingsButton({ company, defaultValues }: Props) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      await saveDableSettings({
        company,
        dable_api_key: fd.get("dable_api_key") as string,
        dable_account: fd.get("dable_account") as string,
      });
        ...
        <>
          ...
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-sm bg-white rounded-2xl shadow-xl border border-gray-100 p-4 z-50 animate-in fade-in slide-in-from-bottom-3 duration-200">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-2.5">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">데이블 API 키</label>
                <input
                  name="dable_api_key"
                  defaultValue={defaultValues?.dable_api_key ?? ""}
                  placeholder="API 키를 입력하세요"
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#0e299c]"
                />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">데이블 계정</label>
                <input
                  name="dable_account"
                  defaultValue={defaultValues?.dable_account ?? ""}
